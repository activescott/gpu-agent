...
test.describe("Filter Items - Price Compare Page", () => {
  test("setting very low budget shows significantly fewer listings", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Shop All GPUs" }),
    ).toBeVisible();
...
test.describe("Filter Items - Ranking Page", () => {
  test("setting very low budget shows significantly fewer GPUs", async ({
    page,
  }) => {
