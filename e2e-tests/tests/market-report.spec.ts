...
test.describe("Market Report", () => {
  ...
  test("displays key takeaways cards", async ({ page }) => {
    ...
    const cards = page
      .locator(".card")
      .filter({ hasText: /Premium|Deal|Drop/i });
  ...
  test("download image button is present", async ({ page }) => {
    ...
    const downloadButton = page
      .locator(".share-menu-dropdown.open")
      .locator("text=Download Image");
  ...
  test("footer links work", async ({ page }) => {
    ...
    const liveButton = page.locator("a.btn-primary", {
      hasText: /Live GPU Prices/i,
    });
    await expect(liveButton).toHaveAttribute("href", "/gpu/price-compare");

    // Check "Compare GPUs" button
    const compareButton = page.locator("a.btn-outline-primary", {
      hasText: /Compare GPUs/i,
    });
...
test.describe("Market Report Chart Data Validation", () => {
  ...
  test("charts do not show insufficient data alerts", async ({ page }) => {
    test.skip(
      !isProduction,
      "Requires historical data only available in production",
    );
    await page.goto(reportPath);
    ...
    if (alertCount > 0) {
      ...
      for (let i = 0; i < alertCount; i++) {
        alertTexts.push(
          (await insufficientDataAlerts.nth(i).textContent()) || "",
        );
      }
  ...
  test("all chart images contain actual data", async ({ request }) => {
    ...
    for (const [chart, minSize] of Object.entries(chartMinimumSizes)) {
      const response = await request.get(
        `/api/images/chart/${chart}?${dateParams}`,
      );
      ...
      if (!passed) {
        console.log(
          `Chart ${chart} image too small: ${body.length} bytes (min: ${minSize})`,
        );
      }
    }
...
test.describe("Market Report Link Validation", () => {
  ...
  test("all relative links return valid responses (not 404)", async ({
    page,
    request,
  }) => {
...
test.describe("Market Report Chart Image API", () => {
  ...
  test("returns valid PNG image", async ({ request }) => {
    ...
    const response = await request.get(
      `/api/images/chart/ScalperPremiumChart?${dateParams}`,
    );
  ...
  test("handles different chart components", async ({ request }) => {
    ...
    for (const component of chartComponents) {
      const response = await request.get(
        `/api/images/chart/${component}?${dateParams}`,
      );
      expect(response.status()).toBe(200);
    }
  });
  ...
  test("line chart image renders correctly", async ({ request }) => {
    const response = await request.get(
      `/api/images/chart/PriceHistoryChart?${dateParams}`,
    );
  ...
  test("returns 404 for unknown chart component", async ({ request }) => {
    const response = await request.get(
      `/api/images/chart/UnknownChart?${dateParams}`,
    );
