...
test.describe("Home Page", () => {
  test("should load and show at least 1 listing under each card header", async ({
    page,
  }) => {
    ...
    await expect(page).toHaveTitle(
      /Find the Best GPU for Your Money|GPUPoet|GPU/,
    );
    ...
    for (let i = 0; i < headerCount; i++) {
      ...
      const listings = container.locator(
        'a[href*="/gpu/shop/"], a[href*="/gpu/price-compare/"], a[href*="/ml/shop/gpu/"], .listing-item, tr:has(a[href*="/gpu/shop/"]), li:has(a[href*="/gpu/shop/"])',
      );
      ...
      expect(
        listingCount,
        `Expected at least 1 listing under header "${headerText}", but found ${listingCount}`,
