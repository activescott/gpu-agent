...
{/* prettier-ignore */}
<Alert kind="success">We track GPU prices and provide rankings for machine-learning cost/performance, such as [GPUs Ranked by Cost per Memory Capacity](/gpu/ranking/ai/memory-gb).</Alert>

The largest memory on a single GPU available, as far as I know, are the [NVIDIA A100](/gpu/learn/card/nvidia-a100-pcie) and the [NVIDIA H100](/gpu/learn/card/nvidia-h100-pcie), each offering 80GB of GPU RAM.

Due to these large memory requirements, usually, the larger models are [quantitized down to lower precision](/gpu/learn/ai/quantization), which can reduce accuracy in some cases but makes them feasible on single-GPU machines.
