import type { MarketReportMetadata } from "../reports"

export const reportMetadata: MarketReportMetadata = {
  slug: "gpu-market-report-march-2026",
  title:
    "GPU Market March 2026: RTX 5060 Dips Below MSRP, RTX 5090 Still 40% Above",
  description:
    "Three RTX 50 cards now have best deals at or below MSRP, but the median buyer still pays 67-93% over. Prior-gen GPUs are cratering: RTX 3070 down 32%, RTX 4070 Ti down 25%. AMD RX 9000 series getting scalped 70-104% above MSRP.",
  publishedAt: new Date("2026-03-01T06:00:00Z"),
  updatedAt: new Date("2026-03-01T06:00:00Z"),
  author: "Scott Willeke",
  tags: [
    "market-report",
    "gpu-prices",
    "rtx-50-series",
    "buying-guide",
    "analysis",
  ],
  dateRange: { from: "2026-02", to: "2026-02" },
}
