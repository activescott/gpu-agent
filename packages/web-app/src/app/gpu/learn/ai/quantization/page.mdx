...
Just to do inference on a large language model (LLM) such as BLOOM-176B, you would need to **have 8x NVIDIA 80GB A100 GPUs**[^3]. Currently the <Link href="/gpu/shop/nvidia-a100-pcie">**NVIDIA A100 80GBs have an average price of <FormatCurrency currencyValue={(await getPriceStats("nvidia-a100-pcie")).avgPrice} />**</Link> on GPU Poet.
...
Because hardware FP8/FP4 support varies significantly by GPU — even within the same product generation — see <Link href="/gpu/ranking/ai/fp8-flops">GPUs ranked by $ / FP8 TFLOP</Link> and <Link href="/gpu/ranking/ai/fp4-flops">GPUs ranked by $ / FP4 TFLOP</Link> to compare cost-performance across cards that support these precisions in hardware.
