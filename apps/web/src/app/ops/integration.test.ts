...
import { resetMetrics, hasMetrics } from '../../pkgs/server/metrics/metricsStore'
...
const mockRevalidateCachedListings = revalidateCachedListings as jest.MockedFunction<typeof revalidateCachedListings>
...
describe('Cache Revalidation Integration', () => {
  ...
  describe('End-to-End Flow', () => {
    it('should complete full revalidation → metrics flow successfully', async () => {
      ...
      expect(initialMetricsText).toContain('coinpoet_last_job_timestamp_seconds 0')
      ...
      expect(updatedMetricsText).toContain('coinpoet_gpus_stale_remaining_total 0')
      expect(updatedMetricsText).toContain('coinpoet_last_job_duration_seconds 8')
    })
    ...
    it('should handle revalidation failure → metrics flow correctly', async () => {
      ...
      mockRevalidateCachedListings.mockRejectedValueOnce(new Error(errorMessage))
    ...
    it('should handle multiple job executions with metrics updates', async () => {
      ...
      mockRevalidateCachedListings.mockRejectedValueOnce(new Error('Network error'))
    ...
    it('should maintain proper HTTP status codes for Kubernetes and Prometheus', async () => {
      ...
      mockRevalidateCachedListings.mockRejectedValueOnce(new Error('Test error'))
      const revalidationResponse = await revalidateCachePost()
      expect(revalidationResponse.status).toBe(500)
    ...
    it('should include proper Content-Type headers for each endpoint', async () => {
      ...
      mockRevalidateCachedListings.mockResolvedValueOnce(mockSuccessResult)
      const revalidationResponse = await revalidateCachePost()
      expect(revalidationResponse.headers.get('Content-Type')).toContain('application/json')

      // Test metrics endpoint returns Prometheus format
      const metricsResponse = await metricsGet()
      expect(metricsResponse.headers.get('Content-Type')).toContain('text/plain')
      expect(metricsResponse.headers.get('Content-Type')).toContain('version=0.0.4')
    })
  })
})