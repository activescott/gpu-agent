...
test.describe("Gaming Buy Pages", () => {
  test("clicking Gaming Benchmarks category from AI page should not 404", async ({
    page,
  }) => {
    ...
    const gamingBenchmarksButton = page.getByRole("button", {
      name: "Gaming Benchmarks",
    });
  ...
  test("should show gaming metrics (not AI specs) on gaming buy pages", async ({
    page,
  }) => {
  ...
  test("should not show AI metrics on gaming buy pages", async ({ page }) => {
    ...
    const specPills = page
      .locator("#listingContainer .badge")
      .filter({ hasText: /\$.*\// });
  ...
  test("should show valid numeric values on all gaming buy pages", async ({
    page,
  }) => {
  ...
  test("should show correct metric for each gaming buy page", async ({
    page,
  }) => {
    const testCases = [
      {
  ...
  test("should redirect old cost-per-* gaming URLs to new slugs", async ({
    page,
  }) => {
