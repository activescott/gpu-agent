...
test.describe("Navigation Links", () => {
  test("all navigation links from homepage should load without errors", async ({
    page,
  }) => {
  ...
  test("navigation links should not use old URL patterns", async ({ page }) => {
    ...
    for (const link of links) {
      ...
      if (
        link.startsWith("/ml/shop/gpu") ||
        link.startsWith("/ml/learn/gpu/ranking")
      ) {
        problematicLinks.push({
          link,
          issue: "Uses old /ml/ path instead of /gpu/",
        });
      }
      ...
      if (link.match(/^ml\//)) {
        problematicLinks.push({
          link,
          issue: "Relative path missing leading /",
        });
      }
      ...
      if (link.includes("/gpu/price-compare/") && link.includes("cost-per-")) {
        problematicLinks.push({
          link,
          issue: "Uses old cost-per-* slug format",
        });
      }
    }
